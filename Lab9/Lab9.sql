...
SELECT AVG(salApr2022) as April, AVG(salMar2022) as March, AVG(salFeb2022) as February,  AVG(salJan2022) as January, AVG(salDec2021) as December , 
AVG(salNov2021) as November, AVG(salOct2021) as October,AVG(salSept2021) as September, AVG(salAug2021) as August, AVG(salJuly2021) as July,   
AVG(salJune2021) as June,   AVG(salMay2021) as May  FROM salary_cal_year
...
official_inf oi on oi.idEmployee=sal.idEmployee group by  surname, id, idDep
...
select idDep, sum(salApr2022) as April2022, sum(salMar2022) as March2022, sum(salFeb2022) as February2022, sum(salDec2021) as December2021, sum(salNov2021) 
as November2021, sum(salOct2021) October2021 from salary_cal_year sal join official_inf oi on oi.idEmployee=sal.idEmployee group by idDep; 

--Какой сотрудник получил наибольшее число отпусков определенного вида? Вернуть для всех видов.
SELECT surname, MainLAbor, AdditionalLabor, ByPregnancy, childcare, GettingEducation, Personal_Family  FROM Holidays join official_inf on 
Holidays.idEmployee=official_inf.idEmployee 
